...
/**
 * Форма удаления аккаунта для модалки. Сам факт открытия модалки — первый шаг
 * подтверждения, внутри запрашиваем ввод username + пароль.
 */
export function DangerZone({
  user, onDeleted, onDone,
}: {
  user: User;
  onDeleted: () => void;
  onDone?: () => void;
  ...
  return (
    <form className={styles.form} onSubmit={handleDelete}>
      ...
      <div className={styles.actions}>
        {onDone && (
          <Button type="button" variant="secondary" onClick={onDone} disabled={mut.isPending}>
            Отмена
          </Button>
        )}
        <Button
